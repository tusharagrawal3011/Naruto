...
pub fn update_rewards(
    ...
    if let Some(u) = user {
        //change pool to a specific amount
...
pub mod nft_staking {
    ...
    pub fn stake(ctx: Context<Stake>) -> Result<()> {
        ...
        // Transfer tokens into the stake vault.
        {
            ...
            user.ninja_mints.push(ctx.accounts.stake_to_account.mint);
    ...
    pub fn unstake(ctx: Context<Stake>) -> Result<()> {
        ...
        // Transfer tokens from the pool vault to user vault.
        {
            ...
            let index = user.ninja_mints.iter().position(|x| *x == stake_to_account_mint).unwrap();
            user.ninja_mints.remove(index);
...
#[derive(Default)]
pub struct Ninja {
    /// The owner of this ninja.
    pub owner: Pubkey,
    /// Name of the Ninja
    pub name: String,
    /// Rank of the Ninja(Genin,Chunin,Jonin)
    pub rank: enum{Genin,Chunin,Jonin},
}
